...
async def extract_audio(song_query: str):
    is_playlist = "list=" in song_query and song_query.strip().startswith(("http://", "https://"))
    ...
    ydl_options = {
        ...
        "quiet": True,
        "no_warnings": True,
        "extract_flat": is_playlist, # Extract only metadata for playlists at first
    ...
    if is_playlist and "entries" in results:
        tracks_to_process.extend(results["entries"])
    else:
        tracks_to_process.append(results)
    ...
    for track_info in tracks_to_process:
        ...
        track = track_info
        if is_playlist or not _has_formats(track):
            try:
                detail_ydl_opts = ydl_options.copy()
                detail_ydl_opts["extract_flat"] = False
                track_url = track.get("url") or track.get("webpage_url")
                if not track_url:
                    continue
                track = await search_ytdlp_async(track_url, detail_ydl_opts)
            except Exception as e:
                log_event("reextraction_error", error=str(e), track_info=track_info)
                continue
