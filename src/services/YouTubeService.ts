        ...
        try {
          if (fs.existsSync(cookiePath)) {
            ...
            logError(
              ...
              new Error(`File not found: ${cookiePath}`),
              {
                cookiePath,
                query,
              },
    ...
    try {
      ...
      return new Promise<string | null>((resolve) => {
        ...
        ytDlp.on("close", (code) => {
          if (code !== 0) {
            logError(
              "Failed to get YouTube stream URL",
      ...
      if (botConfig.ytdlpCookies) {
        try {
          ...
          if (fs.existsSync(botConfig.ytdlpCookies)) {
            const cookieContent = fs.readFileSync(
              botConfig.ytdlpCookies,
              "utf8",
            );
  ...
  spawnPipeStream(source: MusicSource): ChildProcess {
    ...
    if (proxy) {
      ...
      logEvent("youtube_pipe_stream_no_proxy", {
        ...
        url: source.url,
        message:
          "YTDLP_PROXY not set — downloading direct (degraded, may be bot-blocked)",
      });
    }
    ...
    if (botConfig.ytdlpCookies) {
      ...
      try {
        ...
        logError(
          "youtube_pipe_stream: error setting up cookies (ignored)",
          err as Error,
          { cookiePath, title: source.title },
        );
      }
    }
      ...
      if (code !== 0 && signal !== "SIGKILL") {
        ...
        if (stderrBuf.trim()) {
          logError(
            ...
            new Error(stderrBuf.trim()),
            { title: source.title, url: source.url, exitCode: code, signal },
          );
        } else {
